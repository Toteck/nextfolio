...
export function DadosPessoais() {
  ...
  useEffect(() => setCharCount(personalData.summary.length), [personalData.summary]);

  useEffect(() => setExtraErrors((prev) => extraContacts.map((c, i) => prev[i] || "")), [extraContacts]);
  ...
  const updateContact = (id: string, value: string) => {
    const updated = extraContacts.map((c) => (c.id === id ? { ...c, value } : c));
  ...
  const validateField = (field: string, value: string) => {
    switch (field) {
      case "fullName":
        setErrors((prev) => ({ ...prev, fullName: value.trim() === "" ? "Nome obrigatório" : "" }));
        break;
      case "email":
        setErrors((prev) => ({
          ...prev,
  ...
  return (
    <div className="flex flex-col gap-4 space-y-4 mb-10">
      ...
      <InputPhone
        ...
        onChange={(value) => handleChangeAndValidate("phone", maskPhoneDigits(value.replace(/\D/g, "")))}
      />
      ...
      {extraContacts.map((contact, index) => (
        <div key={contact.id} className="flex flex-col gap-1">
          <div className="flex flex-row items-center gap-2">
            ...
            {contact.type === "phone" && (
              <InputPhone
                ...
                value={contact.value}
                onChange={(val) => updateContact(contact.id, maskPhoneDigits(val.replace(/\D/g, "")))}
              />
            )}
          ...
          {extraErrors[index] && <span className="text-red-500 text-sm">{extraErrors[index]}</span>}
        </div>
      ))}
      ...
      <div className="flex gap-2 mt-2 justify-start items-center">
        ...
        <button className="px-2 py-1 text-sm bg-blue-500 text-white rounded hover:bg-blue-800" onClick={() => addContact("email")}>
          + Email
        </button>
        <button className="px-2 py-1 text-sm bg-blue-500 text-white rounded hover:bg-blue-800" onClick={() => addContact("phone")}>
          + Telefone
        </button>
        <button className="px-2 py-1 text-sm bg-blue-500 text-white rounded hover:bg-blue-800" onClick={() => addContact("linkedin")}>
          + LinkedIn
        </button>
        <button className="px-2 py-1 text-sm bg-blue-500 text-white rounded hover:bg-blue-800" onClick={() => addContact("github")}>
          + GitHub
        </button>
      </div>
